...
<xsl:template name="user.head.content">
  ...
  <meta name="generator" content="Experimental LDP.XSL $Revision: 1.3 $"/>
  <xsl:text>
  </xsl:text>
...
<!-- same color as replaceable -->
<xsl:template match="varlistentry/term">
  <span class="term"><font size="-1" color="#006600">
...
<xsl:template match="callout">
  <xsl:choose>
    <xsl:when test="$callout.list.table != 0">
      <tr>
        <td width="5%" valign="top" align="left"><font size="-1">
          <xsl:call-template name="anchor"/>
...
<xsl:template match="varlistentry" mode="xref-title">
  <xsl:apply-templates select="../../../command[1]" mode="xref-to"/> :: <xsl:apply-templates select="term[1]" mode="xref-title"/>
</xsl:template>

<xsl:template match="varlistentry/term" mode="xref-title">
  <xsl:apply-templates/>
</xsl:template>
